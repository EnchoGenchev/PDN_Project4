...
int main(int argc, char* argv[]) {
    ...
    //moved freeing other arrays to after step 3
    free(transactions);
    ...
    for (int i = 0; i < reduced_size; i++) {
        if (reduced_hash[i] < min_hash) {
            min_hash = reduced_hash[i];
            min_nonce = reduced_nonce[i];
        }
    }
    ...
    //moved here since still need to use them in step 3
